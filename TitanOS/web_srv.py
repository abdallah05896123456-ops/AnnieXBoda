# -*- coding: utf-8 -*-
# ==============================================================================
# TITAN OS | ULTIMATE KERNEL (FastAPI Edition - Patched)
# ==============================================================================
...
try:
    ...
    # لن نغلق البرنامج بل سنكمل ليعمل البوت فقط إذا فشل الويب
    pass

# ------------------------------------------------------------------------------
# [2] إعداد المسارات (Path Configuration)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# [3] الربط مع البوت (Bot Bridge) - تم التعديل هنا ✅
# ------------------------------------------------------------------------------
SYSTEM_READY = False
CallClient = None
BotClient = None
...
try:
    ...
    # --- [التعديل الجوهري: البحث عن المشغل الصحيح] ---
    try:
        from AnnieXMedia.core.call import Annie as CallClient
        print("✅ TitanOS: Linked to 'Annie' Controller.")
    except ImportError:
        try:
            from AnnieXMedia.core.call import Call as CallClient
            print("✅ TitanOS: Linked to 'Call' Controller.")
        except ImportError:
            print("⚠️ TitanOS Warning: Controller class (Annie/Call) not found!")
...
async def stream_media(chat_id: str, request: Request):
    ...
    if not file_path: return Response("No Active Stream", status_code=404)
...
async def get_calls():
    ...
    if SYSTEM_READY and CallClient:
        try:
            # محاولة الوصول لقائمة المحادثات النشطة بطرق مختلفة
            active_list = getattr(CallClient, "active_calls", [])
    ...
    if not chats: chats = [{"id": "0", "name": "No Active Signals"}]
...
async def logs(request: Request):
    ...
    if os.path.exists(log_file): return FileResponse(log_file)
...
# ------------------------------------------------------------------------------
def run_uvicorn():
    # تأكد من تركيب: pip install uvicorn
...
def start_server():
    ...
    t.start()
    print("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("🚀 TITAN OS (FASTAPI) IS ONLINE")
