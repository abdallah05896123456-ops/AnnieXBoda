...
async def download_thumb(url: str) -> str | None:
    """يحمل الصورة مؤقتًا ويُعيد مسار الملف، أو None."""
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=20) as resp:
                ...
                tmpf.write(content)
                tmpf.close()
                # تصغير الصورة ليتوافق مع متطلبات Telegram (max 200KB/320x320 أفضل)
                resized = tmpf.name + "_res.jpg"
                # استخدام ffmpeg لخيار سريع وآمن (سيكون سريع ولا يعيد ترميز الصوت)
                try:
                    cmd = [
                        "ffmpeg", "-y",
                        "-i", tmpf.name,
...
async def fast_remux_to_m4a(src_path: str) -> str:
    """لو الملف بالفعل m4a يرجعه، وإلا يعمل remux (copy codec) ويعيد المسار النهائي.
    يستخدم ffmpeg لكن مع stream copy لذا سريع جدًا.
    """
...
def get_duration(path: str) -> int:
    """استخراج المدة (بالثواني) باستخدام mutagen — سريع وخفيف."""
    try:
        audio = MP4(path)
...
async def fast_send_m4a(client, mystic_msg, chat_id: int, src_file: str, title: str, duration_sec: int, user_name: str, thumbnail_url: str | None = None, filename: str | None = None):
    """المُعالج الذي يقوم بـ:
    - اعادة تغليف لمصلح m4a بدون إعادة ترميز (سريع)
    - تحميل thumb وتصغيره
    - ارسال كـ document مع thumb (سريع وأسرع من send_audio في معظم الحالات)
...
@app.on_message(filters.command(["فتح انلاين البحث", "فتح انلاين بحث"], prefixes=["", "/"]) & filters.user(SUDO_IDS))
...
@app.on_message(filters.text & filters.regex(r"^/?(اغنية|اغنيه|هات|هاتلي|ابعتلي|song|video|تحميل|يوتيوب)(?:\s+(فيد|فيديو|video))?(?:\s+(.+))?$") & ~BANNED_USERS, group=5)
