...
# ✅ التعديل الضروري: استدعاء المكتبة الحديثة المتوافقة مع السيرفر
...
async def inline_query_handler(client, query):
    ...
    if text.strip() == "":
        ...
        try:
            # ✅ البحث باستخدام المكتبة الجديدة (Future Lib)
            search = VideosSearch(text, limit=20)
            data = await search.next()
            result = data.get("result", [])
            
            # حلقة تكرار ذكية تتوقف لو النتائج أقل من 15
            for x in range(min(15, len(result))):
                try:
                    ...
                    # الرسالة المطولة والمنسقة (نفس تنسيقك)
                    searched_text = f"""
