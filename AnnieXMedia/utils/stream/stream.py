...
# Hybrid Edition: Annie Base + Brandrd Safety + Alexa Logic
import os
import asyncio
...
# === Helper for Safe Deletion (From Brandrd) ===
...
async def stream(
    ...
    # Ensure Boolean Logic (From Alexa/Brandrd)
    ...
    if streamtype == "playlist":
        ...
        for search in result:
            ...
            if await is_active_chat(chat_id):
                ...
                # Critical Download Step
                try:
                    file_path, direct = await YouTube.download(
                        vidid, mystic, video=is_video, videoid=vidid
                    )
                except Exception:
                    # If single track fails in playlist, skip it
                    continue
                ...
                # FloodWait Protection (From Brandrd)
                try:
    ...
    # ==========================
    # 2. YOUTUBE MODE (The Main Fix)
    # ==========================
    elif streamtype == "youtube":
        # Safer Dictionary Access (From Brandrd)
        ...
        try:
            # This calls the Fixed YouTube.py
            file_path, direct = await YouTube.download(
                vidid, mystic, video=is_video, videoid=vidid
            )
    ...
    # ==========================
        ...
        # Fallback for Duration Key (Fixes common error)
