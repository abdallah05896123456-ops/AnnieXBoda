...
# معالج العمليات الشامل: حل مشكلة البوت + فك تشفير الجودات + السرعة القصوى
# التقنيات: 16 نواة + Aria2c + RAM Disk + Remote EJS + Metadata Fix
...
class Config:
    # استخدام الرام ديسك لضمان أعلى سرعة قراءة وكتابة
    DOWNLOAD_PATH = "/dev/shm/AnnieDownloads" if os.path.exists("/dev/shm") else "downloads"
    # تمويه المتصفح لتجاوز نظام حماية يوتيوب
...
class YTProcessorAPI:
    ...
    def get_cookie_file(self):
        """جلب ملف الكوكيز لفك حظر تسجيل الدخول"""
    ...
    async def get_quality_buttons(self, vidid, stype):
        """حل مشكلة اختفاء الجودات عبر الريموت والعملاء الموثوقين"""
        yturl = f"https://www.youtube.com/watch?v={vidid}"
        
        # إعدادات تخطي حماية يوتيوب الحديثة
        ydl_opts = {
            "quiet": True,
            ...
            "extractor_args": {
                "youtube": {
                    "player_client": ["web", "ios"], # استبعاد أندرويد لتجاوز الحظر
                    "skip": ["dash", "hls"]
                }
            }
        }
        ...
        try:
            ...
            # في حال الفشل التام، يظهر زر جودة افتراضية لضمان عمل البوت
    ...
    async def download_file(self, url, format_id, is_video, title):
        """التحميل بـ 16 اتصال Aria2c لضمان أقصى سرعة"""
    ...
    async def send_smart_file(self, client, chat_id, file_path, is_video, title, duration, thumb, user_name):
        """إرسال الملف مع تثبيت العنوان والاسم الصحيحين"""
        if not os.path.exists(file_path):
            return False

        from AnnieXMedia import userbot
        # الردود بأسلوب بسيط ومنسق
        ...
        try:
            ...
            LOGGER(__name__).error(f"خطأ في الإرسال: {e}")
            return False
